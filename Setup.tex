...
\section{Setup}\label{sec:setup}
	...
	% Fig.\ref{fig:SetupProtoISSim}-\ref{fig:} show the SIMION models of the Prototype ion-source, the filament housing, the ion-mirror and the detector. Fig.\ref{fig:SetupProto} shows the NIM Prototype as it was at the beginning of the measurement campaigns. Detailed cable schema and connection to the laboratory power supplies can be found in \cite{Diss_Meyer} Chap. 4.2.
		...
		% First description of the whole model according to the picture with all PFM electrodes and then point out the differences between the 2 designs (IS, Fil and reflectron)
		\begin{figure}[h]
			\centering
	...
	\subsection{Test facilities/ Test Tools \notes{Draft}}\label{subsec:setupTestTools} % Bei test tools wäre das simulationsprogramm auch inbegriffen
...
%------------------------------------------------------------------------------------
		...
		% the following part may only will show up in the Appendix because its only a derivation used in the lab. Except the electrical schematics of the detector with the corrsponding Variables.
		% Arrange Figrues as soon as there is enough text in the Setup section. At least the electric circuit should appear as close as possible in this section at the end!!!
		...
		% Where do we put the whole detector discussion in? Is that part of the measurements? Or of the Setup because it is part of the instrument. The Prototype had most certainly a diode but did never cause any problems... A comparison of the Prototype and PFM detector is unnecessary.

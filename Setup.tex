...
%---------------------------------------------------------------------------------------------------------------
		...
		\subsection{Detector}\label{subsubsec:SetFacPumpst} % May merge that chapter with the previous one.
		...
		% Detector suffered still discharges -> resistor instead of diode due to loose contact and different number of gold rings for each detector version.
		...
		% This design is still not perfect thus from the electrical point of view the connection is still not very good. Another solution may would be to use a spring between the screw and the MCPs which garantees a good connection between the contact lug and the MCPs. (Reference to Rosina/ ROSETTA?)
		...
		% With the flight electronics it is not possible to determine the current flowing through the MCPs an thus the MCP voltage. Therefore, a useful calibration is needed -> calculation with lab equipment. Differnt subchapter or put all in one as with the pulser discussion? Make it clear in the intro to the chapter...

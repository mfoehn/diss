...
%--------------------------------------------------------------------------------------------
	\subsection{Pulser} \label{chap:ExpPulser} % May explain here again the different parameters and why they matter?
...
%--------------------------------------------------------------------------------------------
	...
	% When a discharge suddenly lowers the applied acceleration voltage, these free electrons get pushed towards the channels walls due to space charge.
	These electrons destroy the conductive coating of the channels leading to a signal reduction of about 30~\% per discharge. The higher the applied MCP voltage is, the higher is the signal loss.\\
	...
	% Old values: Uinit 6 eV, A = 0.3 eV
...
%-----------------------------------------------------------------------------------------------		
		...
		% This is a bit a better performance than with the PFM. Reasons lay in the better voltage set found for the FS instrument and also in the small differences of the manufacturing thus not every instrument is exactly the same. This was very good visible when optimising because a few voltages were extremely different between the PFM and the FS instrument when optimising the system namely IS~1. IS~1 is the electrode opposite of 
...
%-----------------------------------------------------------------------------------------	
		\textbf{TVT}\\ % Useful title? More details about the TVT and how it works? Or leave the main focuse on the importance of the optimiser?
		In the following section a short summary of the thermal vacuum test of the NIM FS model is given. During the whole mission, the JUICE spacecraft guarantees with survival heaters that the temperature of NIM does not fall below \notes{Temp!} to guarantee the survival of all components. This set the minimal temperature at which NIM has to operate. During operation, the electronics warms up and certain components reach temperatures in the range of 100° \notes{Check Temp according to TVT report}. This temperature is the maximal temperature at which NIM has to function. Due to the thermal design with radiators at the electronics the electronics continuously emits power to the outer space to cool the system. % is on desktop.
		Therefore, NIM has to be thermally cycled and on every plateau, NIM has to conduct measurements to verify its functionality when operated at the corresponding temperature.\\ % Check the exact procedure from the TVT report to write it a bit clearer with more explanations why the TVT is necessary and what is the purpose of it. Why 5 cycles if that is written somewhere on a public report. Otherwise ask tomorrow someone.
		...
		In the laboratory the optimisation can be done manually. In space that is not possible. Therefore, a software is needed optimising the voltage sets frequently % How often
		during flight to allow readjustment of the voltages to compensate the effects of the voltage drifts and movement of the mechanics. % State that the optimiser also was successfully used in the ROSETTA mission.
		\begin{figure}[h!]
			\centering
		...
		% N2 was better visible and more stable than the water peak and was thereofre taken as the reference for that analysis instead of H2O.

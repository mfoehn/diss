...
%--------------------------------------------------------------------------------
	\subsection{Simulations \notes{Only if there is time to write something. Material is available}}
...
%--------------------------------------------------------------------------------------------
	...
	% Pumpstand nr.2 (Chapter \ref{subsubsec:SetFacPumpst}).
	% describe the advantage of the anode vs. the resistor (constant voltage but resistor proved to be more robust against discharges.)
	\begin{comment}
	% This text is part of the setup now. It is only here as reminder what is written there.
	...
	\begin{figure}[h]
		...
		\caption{Dependence of the MCP voltage (U\textsubscript{MCP}) from the voltage applied over the detector (U\textsubscript{stack})} % fit parameters: ax +b; a = 0.932, b = 12 V
		\label{fig:PFMUstackUmcpTimeEvol}
	\end{figure}
	\begin{figure}[h] % Flat and folded detector. Lineal is missing on the figures.
		\centering
	...
	\begin{figure}[h] % Theoretical gain curve as a function of time
		...
		\caption{Relative gain of an MCP as a function time.} % Which time
	...
	% Put this part into the Setup?
	%	During the tests, the detector suffered repeatedly discharges leading to a mechanical redesign of the detector housing (Chap.\,\notes{Ref!!!}). In addition, the the diode was replaced by a resistor.
	%	Fig.\,\ref{fig:PFMUstackUmcpTimeEvol} shows the relationship between the voltage $U_{stack}$ and $U_{MCP}$.
	% Write something why we used a flex PCB for that purpose.
	The flight detectors consists of a flexible PCB (Printed Circuit Board) with a peek housing for the MCPs. The flex PCB was needed to reduced the detector volume and therefore to reduced the volume, which has to be shielded against the strong radiation in Jupiter's radiation field. The detectors are tested in flat configuration with test MCPs and afterwards in folded configuration with flight MCPs (Fig.\,\ref{fig:DetFlatFolded}). The detectors were put in a vacuum chamber and baked out for 48~h. Earliest after another 24~h after bake out, the tests were started. The conditioning procedure is attached in the Appendix (Ref.\,\notes{Add MCP conditioning procedure}). Depending on the outgas behaviour of the detector, the conditioning took 2.5 to 4.5~h. After the conditioning, the measurements of the detector gain were performed. Fig.\ref{fig:SN4p54p7Gain}~left shows the gain curves of two FS detectors in the two configurations. The conditioning and the measurements in the folded configuration were only done up to lower voltages to spare the MCPs. The difference in gain between the two detectors is explained by that the conditioning of SN~4p7 took 1~h longer than with detector SN~4p4.
	% Start discussion how the theoretical gain curve looks like as a function of time and detector usage. Just mention that that explains the difference between the curves of the 2 detector. That you have a variance in the conditioning and especially in the folded state, SN 4p4 was 3 days longer in vacuum than 4p7 and had therefore more time to outgase.
	% Discussion about the discharge. Theory? or just mention that for every discharge, there is a gian loss of about 30% check if that matches the recorded curves.
	% Gain curve in the instrument. Explain the different paramters? Are already explained in the theory chapter. What else? That the gain curve matches well the data? with Uinit ca. 6 eV and A 0.3 (1/ev)^(1/2)? The data is normed to hydrogen. Can we tell from that how Kr and the other things scale? No. because also the ionisation efficiency has an impact on the gain/ on the amount of gas seen by the spectrometer.

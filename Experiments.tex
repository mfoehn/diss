...
%---------------------------------------------------------------------------------------------------------
	\subsection{Entrance adaption} % Ion and e- distribution in the ionisation region. Entrance adaption
	In the following section we look where the ions in the ionisation region have to be generated that they actually reach the detector and we look at the distribution electrons which are used to ionise the neutral particles.
	
	\begin{comment}
		\begin{figure}[h] % PFM entrance side and Top view. enlarge picture incase there is enough text.
			\centering
			\includegraphics[width=\textwidth]{Experiments/PFMEntrance_SideTop_View.png}
			\caption{Side view (left) and top view (right) of the entrance slit.}
			\label{fig:PFMentrSideTopSchema}
		\end{figure}
	\end{comment}
	...
	\begin{figure}[h] % Entrance schema.
		...
		%	\caption{Top view}
		\end{subfigure}
		\caption{Side (top) and top view (bottom) of the entrance slit. The violet arrows mark the influx of the neutral gas and the blue arrows mark the electron beam.}
		\label{fig:PFMentrSideTopSchemas}
	\end{figure}


	\begin{figure}[h] % Sim ions which reach the detector. Caption
		\begin{subfigure}[t]{.5\textwidth}
			\centering
		...
		\end{subfigure}
		\caption{Simulated ions marked as small arrows starting at different positions in the entrance slit. The ions in n-mode enter the entrance with 4~km/s. The blue ions reach the detector where the orange ions hit the structure during their flight to the detector.}
		\label{fig:PFMentrSideTopSim}
	\end{figure}
	\begin{figure}[h]
		\centering
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=.7\textwidth]{Experiments/IS_adaption_TopView.png}
	...
	Fig.~\ref{fig:PFMentrSideTopSchemas}~top shows the ionization region from the side. The antechamber is on the top where the neutral particles from the thermal mode come from (violet arrow) and the generated ions will be pulled into the spectrometer with the inner pulser grid $IS~5$. The violet arrows mark the inflow of the neutral gas once coming from the antechamber (th-Mode) and once coming from the side (n-Mode). The blue arrow marks the flight direction of the electrons emitted by the filament to ionise the neutral gas. Fig.~\ref{fig:PFMentrSideTopSchemas}~bottom shows the top view of the ionisation region when looking from the antechamber into the spectrometer. The entrance slit $IS~3$ has a special form because on the right side, the two filaments reach into the source and bloc part of the field-of-view and on the left side there is supporting structure reaching into the source to support the antechamber from the other side. In n-Mode, the gas can enter the ionisation region from every direction in the yz-plane except from where the filament and the supporting structure are resulting in a field-of-view of 300°. For the lab measurements, the gas inflow direction is usually from the bottom as it is also shown in Fig.~\ref{exp:PFMIntCharEnt}. For the simulations, a gas inflow direction parallel to the electron beam was chosen as it is marked in Fig.~\ref{fig:PFMentrSideTopSchemas}~bottom.\\
	Here we were interested in where the ions have to be generated in th- and n-mode to actually reach the detector. The ions were generated in a cuboid volume with a height of 2~mm because that is the height of the entrance slit and with a square base area of 8x8~mm. Fig.~\ref{fig:PFMentrSideTopSim} shows the simulated data left for thermal mode where the particles have thermal velocity and right for the neutral mode where the incoming particles have a velocity of 4~km/s coming from negative z-direction. The particles are displayed as velocity vectors. The ions marked in blue are the ones reaching the detector and the orange ones hit the structure somewhen during their flight to the detector. The simulated particles had unit masses from 1 up to 1000~m/z. The pictures show nicely that only ions reach the detector which are generated above the inner extraction grid $IS~5$ which is the one the extraction pulse is applied for ion extraction. In thermal mode, also some ions generated over the outer grid reach the detector. They are visible as a small blue ring close to the structure separating the inner and the outer grid ($IS~5$ and $IS~6$) in Fig.~\ref{fig:PFMentrSideTopSimThTop}). In thermal mode the neutral particle flow comes through the pink structure and is directed over the inner pull grid. Only a small amount of gas reaches the outer grid. In neutral mode, only ions generated over the inner grid reach actually the detector due to their velocity perpendicular to the x-axis of the instrument.\\
	In the next section, we look at the flight path of the electrons which are used to ionise the neutral gas. During vibration tests of the sensor, a weakness in the structure of the ion source was identified leading to a small redesign. The opening for the electron beam in the entrance electrode $IS~3$ had to be enlarged (Fig.~\ref{fig:IsAdapt}). Simulations were done to see how big the impact of that design adaption was. Fig.~\ref{fig:IsAdapt} bottom panel shows the old design with an additional ring to reduce the diameter of that hole and the top panel shows the new design without the additional ring in the entrance electrode. Dark blue is the flight path of the electrons and light blue shows the potential lines. The adaption of the corresponding electrode had not big impact on the flight path of the electrons. Fig.~\ref{fig:IsAdaptTop} shows the top view from the entrance. The porous structure is the outer extraction grid $IS~6$. The inner grid is covered by the electron beam. In the simulation model, only one filament was simulated because it was sufficient for these simulations. Between the two potentials line which mark the 50~V ring is the electrode $IS~4$. We see nicely in Fig.~\ref{fig:IsAdapt} and Fig.~\ref{fig:IsAdaptTop} that the electron beam covers the whole volume over the inner extraction grid.

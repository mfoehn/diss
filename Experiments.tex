...
%-------------------------------------------------------------------------------------	
	...
	\subsection{Shutter performance test} % Not a much material. Similar to reflectron. its a verification and showing of the limitations of the possible test.
	...
	% Motor tests:
	% Factor 7 in the lab instead of 1000. Gas scatters in the chamber and finds other ways to reach the entrance. To test is accurately, one has to channel the gas in the chamber and suppress the scattering. When subtracting the hobby background and calculate it with the IsoArea the factor is about 11. IsoArea corresponds to the number of detected particles. Gas flows around the instrument and leaves the environment. Why is no good background subtraction possible?...
	% Difference between real background and the hobby background? Gas leaves and does not flow back. Ram pressure.
	% Leave away and argument that particles scattering in the vacuum chamber also contribute to the signal. This amount is difficult to determine because for a really realistic scenario one has to remove the scattered particles and work with a much wider beam.
	% this test can just give a confirmation if the shutter works but does not say anything about the quality. Lower boundary approximation?...
	% Lot of background signal... Tried to estimate it with a background measurement. Does not fully represent the state...
	% Imperfect voltage set? Reason for everything.
	% Neutral mode the amount is much less (why? Estimation?)
	% May delet row with the background
	
	% Measurements were done with PFM! Not with Prototyp. State that clearly.
	% Folder with measurement data:
	% \\titania.unibe.ch\UserHomes\foehn\My Documents\PhD\Messungen\PFM\PFM_1905to1911\Motortest_191014

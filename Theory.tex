...
%----------------------------------------------------------------------------------------------------------------------
	\subsection{Filament \notes{Draft}}
		% Describe here the requirements for the filament (operation time, enhanced coating, longer wires (Ref. to Pic) vib test, conditioning). Give only a short summary with reference to Rico's diss because there the longe term test is explained in detail. Only summarize here the key points relevant for NIM.
		% Followed by the power estimation accoring to a very rough estimations and the plot with the EM and flight board showing that the boards are almost able to provide the needed power.
		
		% Mention expected minimal lifetime for the filaments for the mission.
...
%---------------------------------------------------------------------------------------------------------------------------
		...
		The Y\textsubscript{2}O\textsubscript{3} is heated up until it emits electrons. The required temperature depends on the emission current. The Richardson law gives the emission current density $j$ for a material as a function of the temperature $T_d$:
		\begin{equation}
		...
		% In this chapter the performance of the PFM and FS filament controller boards is discussed. First we discuss a theoretical curve and later on we show the performance of the PFM and FS board with different filaments.\\
...
%		\textcolor{red}{see also density enhancement book for that derivation!!!}
	...
	\subsection{Detector Parameters \notes{Draft}}\label{sec:DetParam} % Clean up this chapter when finalizing the detector chapter.
	...
	% State here clearly how the MCPs work with the electron avalange. May also with a pitcure.
	...
	\textbf{Gain}\\
	\begin{figure}[h] % Schema of the MCP detector. May explain it a bit better in the text to be able to reference it in the Setup.
		\centering

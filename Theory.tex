...
% Short overview with references to the different chapters which analyse certan parts a bit more in detail. Part of the Intro? Can still be move into the intro part.
	...
	%------------------------------------------------------------------------------------------------
	\subsection{Mass Resolution \notes{Discussion Measurement missing}}\label{chap:massRes}
...
%--------------------------------------------------------------------------------------------------	
	...
	\subsection{Filament \notes{Draft}}
		...
		% This calculation sets the needed power of the filament. The power is then used in the next chapter to show where on the line we are.
		...
		% Only do the life time estimation for the Y2O3 part because its the only thing publicly known. The thickness is also not public... Have a look how it would work. Short literature search about these estimations and not about the factor. When this chapter is finished.
		
		In this chapter shows three different ways to estimate the life time of the filament. In the first approximation we look how long it takes for the W3Re to sublimate. In a water environment tungsten forms tungsten oxid with sublimates. In the 3 approximation we look how long it take for the coating material Y\textsubscript{2}O\textsubscript{3} to sublimate.
		\begin{figure}[h]
			\centering
		...
		\begin{figure}[h]
			...
			\label{fig:FilRico}
		\end{figure}
		% include the FS data. Remove the EM Data? Because it is not so relevat for the Discussion. Just make a comparison between the PFM and the FS.
		% Continue with errobars for the symbole.
